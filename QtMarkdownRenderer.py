...
class QtMarkdownRenderer(mistune.Renderer):
	...
	def link(self, link, title, text):
		...
		if "://" not in link and link.endswith(".json") :  # Link to a Json file
			...
			cmd = "<a href=file:\"{link}\"><font color=\"{colour}\">{text}</font></a>".format(colour=link_colour, link=link, text=text)
			# UM.Logger.Logger.log("i", "File cmd: {cmd}".format(cmd=cmd))
			return cmd
	...
	def condition_met(cls, condition):
		...
		for match in re.finditer(check_pattern, condition):
			...
			if variable == "cura_version":
				...
				# Remove any extra version numbers. They mess with the comparison.
				variable_value = UM.Version.Version(str(variable_value.getMajor()) + "." + str(variable_value.getMinor()) + "." + str(variable_value.getRevision()))
